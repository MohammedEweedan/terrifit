...
export default function EditProfileScreen() {
  ...
  const [avatar, setAvatar] = useState<{ avatarImage: string | null; avatarEmoji: string | null }>({
    avatarImage: profile?.profile?.avatarImage ?? null,
    avatarEmoji: profile?.profile?.avatarEmoji ?? null,
  });
  ...
  return <KeyboardAvoidingView style={s.page} behavior={Platform.OS === "ios" ? "padding" : undefined}>
    <ScrollView keyboardShouldPersistTaps="handled" contentContainerStyle={[s.content, { paddingBottom: insets.bottom + 30 }]}>
      <ModalHeader
        title="Profile"
        right={<Pressable disabled={saving} onPress={() => void save()} hitSlop={10}><Text style={[s.save, saving && s.disabled]}>{saving ? "Saving" : "Save"}</Text></Pressable>}
      />
      <Text style={s.eyebrow}>Make it yours</Text>
      <View style={s.avatarRow}>
        <AvatarPicker image={avatar.avatarImage} emoji={avatar.avatarEmoji} name={name} onChange={setAvatar} />
      </View>
