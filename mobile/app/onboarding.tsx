...
/**
 * Preferences first, market second, then the questions.
 *
 * The order is deliberate: the look and the language are set before anything
 * is asked, so the questions arrive in the language the person actually chose
 * rather than the one the phone guessed.
 */
const FIRST_STEP = -2;
const LAST_STEP = 7;
...
export default function OnboardingScreen(){
  ...
  const total=LAST_STEP-FIRST_STEP+1;
  ...
  const valid=useMemo(()=>{
    ...
    if(step===1){const age=Number(form.age),h=Number(form.height),w=Number(form.weight);return age>=13&&age<=100&&h>0&&w>0;}
    ...
    return true;
  },[step,form,preferences.countryCode]);
  ...
  return <KeyboardAvoidingView style={s.flex} behavior={Platform.OS==="ios"?"padding":undefined}><View style={[s.shell,{paddingTop:insets.top+12,paddingBottom:Math.max(insets.bottom,16)}]}>
    <View style={s.top}><Pressable disabled={step===FIRST_STEP} onPress={()=>setStep(v=>Math.max(FIRST_STEP,v-1))}><Text style={[s.back,step===FIRST_STEP&&s.hidden]}>{preferences.t("back")}</Text></Pressable><TerrifitMark size={20}/><View style={s.topSpacer}/></View>
    <View style={s.track}>{Array.from({length:total},(_,i)=><View key={i} style={[s.tick,i<=step-FIRST_STEP&&s.tickOn]}/>)}</View>
    <ScrollView style={s.scroll} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      ...
      {step===3?<Step eyebrow="Training load" title="Meet your real week." body="We’ll start here and adjust when your body says the plan is too easy—or too much."><View style={s.options}>{LEVELS.map(([id,title,body])=><Choice key={id} active={form.activityLevel===id} title={title} body={body} onPress={()=>setForm({...form,activityLevel:id})}/>)}</View><Text style={s.miniLabel}>Training days per week</Text><View style={s.dayRow}>{[0,1,2,3,4,5,6,7].map(day=><Pressable key={day} onPress={()=>setForm({...form,trainingDays:day})} style={[s.day,form.trainingDays===day&&s.dayOn]}><Text style={[s.dayText,form.trainingDays===day&&s.dayTextOn]}>{day}</Text></Pressable>)}</View></Step>:null}
      {step===4?<Step eyebrow="Health context" title="Anything we should respect?" body="This improves safety prompts and recommendations. It never appears on your public profile."><View style={s.chips}>{CONDITIONS.map(x=><Chip key={x} active={form.conditions.includes(x)} label={x} onPress={()=>toggle("conditions",x)}/>)}</View><View style={s.private}><Text style={s.privateTitle}>Private by default</Text><Text style={s.privateBody}>Health conditions stay encrypted in your account and are never sold or shown to other members.</Text></View></Step>:null}
      ...
      {step===7?<Step eyebrow="Ready" title="Your Terrifit starts now." body="Your home feed, Maps and daily guidance will be shaped around these choices."><View style={s.summary}><Summary label={preferences.t("market")} value={`${preferences.countryName()} · ${preferences.currencyCode}`}/><Summary label="Goal" value={GOALS.find(x=>x[0]===form.goal)?.[1]??"Set later"}/><Summary label="Weekly training" value={`${form.trainingDays} days`}/><Summary label="Activities" value={form.activities.slice(0,3).join(" · ")||"Set later"}/><Summary label="V1" value={form.v1Mode==="real"?"Pair physical band":"Connect later"}/></View><Text style={s.disclaimer}>Terrifit provides wellness guidance, not medical diagnosis. If a condition changes how you can safely train, work with a qualified clinician.</Text></Step>:null}
    </Animated.View>
    </ScrollView>
